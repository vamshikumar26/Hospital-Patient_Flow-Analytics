...
gold_dim_patient = "abfss://<<containername>>@<storageaccount>>.dfs.core.windows.net/dim_patient"
gold_dim_department = "abfss://<<containername>>@<storageaccount>>.dfs.core.windows.net/dim_department"
gold_fact = "abfss://<<containername>>@<storageaccount>>.dfs.core.windows.net/fact_patient_flow"

# Read Data
silver_df = spark.read.format("delta").load(silver_path)

# Defining Window for latest admission per patient
w = Window.partitionBy("patient_id").orderBy(F.col("admission_time").desc())

#Rank by latest admission_table
#Keep only latest row
silver_df = (
    silver_df.withColumn("row_num",F.row_number().over(w)).filter(F.col("row_num") == 1).drop("row_num")
)

#Patient Dimension Table Creation
#Preparing incoming dimension records (duplicated per patient, latest record)
incoming_patient = (
    silver_df.select("patient_id","gender","age").withColumn("effective_from",current_timestamp())#to track the status of particular record
)

#create target if not exists
if not DeltaTable.isDeltaTable(spark,gold_dim_patient):
    #intialize table with empty data and schema
...
#create an expression to detect attribute changes(hash or explicit comparison)
#we use hash to detect changes
...
#create view for merging dataframes
